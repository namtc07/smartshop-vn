      ...
      <div className="relative w-full sm:max-w-lg bg-zinc-900 border border-zinc-800 rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Kem dưỡng da ban đêm Pro-X..." className={inputClass} />
          </div>

          {/* Original URL */}
          <div>
            <label className={labelClass}>Link sản phẩm gốc <span className="text-red-500">*</span></label>
            <input type="url" value={form.originalUrl} onChange={e => setForm(f => ({ ...f, originalUrl: e.target.value }))} placeholder="https://shopee.vn/product/..." className={inputClass} />
          </div>

          {/* Affiliate Deep Link */}
          <div>
            <label className={labelClass}>Affiliate Deep Link <span className="text-red-500">*</span></label>
            <input type="url" value={form.affiliateDeepLink} onChange={e => setForm(f => ({ ...f, affiliateDeepLink: e.target.value }))} placeholder="https://s.shopee.vn/affiliate/..." className={inputClass} />
